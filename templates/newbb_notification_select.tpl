<{if $xoops_notification.show}>
    <form name="notification_select"
          action="<{$xoops_url}>/modules/<{$xoops_dirname}>/<{$xoops_notification.target_page}>" method="post">
        ...
        <table class="outer">
            ...
            <{/foreach}>
            <tr>
                <td class="foot txtcenter" colspan="3"><input type="submit" name="not_submit"
                                                              value="<{$lang_updatenow}>"/></td>
            </tr>
        </table>
        <div class="txtcenter">
            <{$lang_notificationmethodis}>:&nbsp;<{$user_method}>&nbsp;&nbsp;[<a href="<{$editprofile_url}>"
