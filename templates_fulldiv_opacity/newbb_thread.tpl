...
<div class="thread_title_bar <{if $topic_post.post_no == 1}>thread_title_bar_top<{/if}> outer"
...
<div class="thread_body forum_table">
    <div class="thread_userinfo odd forum_cell">
        <{if $topic_post.poster.uid gt 0}>
            ...
            <div class="thread_poster_rank comUserRankText"><{if $topic_post.poster.rank.title !=""}> <{$topic_post.poster.rank.title}>
                    <br>
                    <imgsrc
                    ="<{$xoops_upload_url}>/<{$topic_post.poster.rank.image}>" alt="<{$topic_post.poster.rank.title}>" /><{/if}>
            </div>
            <{if $topic_post.poster.avatar != "blank.gif"}>
                <br>
                <img class="thread_poster_img comUserImg" src="<{$xoops_upload_url}>/<{$topic_post.poster.avatar}>"
                     alt="<{$topic_post.poster.name}>"/>
            <{else}>
                <br>
                <{$anonym_avatar}>
            <{/if}>
            <{if $infobox.show}>
                ...
                <div id="toggle-<{$topic_post.post_id}>" class="toggle_<{$infobox.style}>">
                    <div class="thread_poster_stat comUserStat"><span
                                class="thread_poster_stat_caption comUserStatCaption"><{$smarty.const._MD_JOINED}>
                            :</span><br><{$topic_post.poster.regdate}>
                        <br><span class="thread_poster_stat_caption comUserStatCaption"><{$smarty.const._US_LASTLOGIN}>
                            :</span><br><{$topic_post.poster.last_login}></div>
                    <{if $topic_post.poster.from}>
                        <div class="thread_poster_stat comUserStat"><span
                                    class="thread_poster_stat_caption comUserStatCaption"><{$smarty.const._MD_FROM}></span> <{$topic_post.poster.from}>
                        </div>
                    <{/if}>
                    ...
                    <div class="thread_poster_stat comUserStat">
                        <span class="thread_poster_stat_caption comUserStatCaption"><{$smarty.const._MD_POSTS}>:</span>
                        ...
                        <{if $topic_post.poster.digests gt 0}>
                            |
                            <span class="thread_poster_stat_caption comUserStatCaption"><{$smarty.const._MD_DIGESTS}>
                                :</span>
