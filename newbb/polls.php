...
if (is_object($pollModuleHandler)  && $pollModuleHandler->getVar('isactive')) {
	...
	if($pollModuleHandler->getVar("version") >= 140) {
		...
		xoops_load('request', $xoopsModuleConfig["poll_module"]);
		xoops_loadLanguage('admin', $xoopsModuleConfig["poll_module"]);
...
if (!newbb_isAdmin($forum_obj)  && is_object($GLOBALS['xoopsUser'])) {
    $perm = false;
...
switch ($op) {
	case "add":
		...
		// old xoopspoll or umfrage or any clone from them
			...
			$barcolor_array = XoopsLists::getImgListAsArray(XOOPS_ROOT_PATH . "/modules/".$xoopsModuleConfig["poll_module"]."/images/colorbars/");
			for($i = 0; $i < 10; $i++) {
				...
				$color_select->setExtra("onchange='showImgSelected(\"option_color_image[{$i}]\", \"option_color[{$i}]\", \"modules/".$xoopsModuleConfig["poll_module"]."/images/colorbars\", \"\", \"" . XOOPS_URL . "\")'");
				$color_label = new XoopsFormLabel("", "<img src='" . XOOPS_URL . "/modules/".$xoopsModuleConfig["poll_module"]."/images/colorbars/" . $current_bar . "' name='option_color_image[{$i}]' id='option_color_image[{$i}]' width='30' align='bottom' height='15' alt='' /><br />");
	...
	case "edit":
		...
		// old xoopspoll or umfrage or any clone from them
			...
			$poll_form->addElement($notify_yn);
			$option_tray = new XoopsFormElementTray(_MD_POLL_POLLOPTIONS, "");
			$barcolor_array = XoopsLists::getImgListAsArray($GLOBALS['xoops']->path("modules/{$xoopsModuleConfig['poll_module']}/images/colorbars/"));
			$i = 0;
			foreach ($options_arr as $option) {
				...
				$color_select->addOptionArray($barcolor_array);
				$color_select->setExtra("onchange='showImgSelected(\"option_color_image[{$i}]\", \"option_color[".$i."]\", \"modules/{$xoopsModuleConfig['poll_module']}/images/colorbars\", \"\", \"" . XOOPS_URL . "\")'");
				$color_label = new XoopsFormLabel("", "<img src='" . $GLOBALS['xoops']->url("modules/{$xoopsModuleConfig['poll_module']}/images/colorbars/" . $option->getVar("option_color", "E")) . "' name='option_color_image[{$i}]' id='option_color_image[{$i}]' class='alignbottom' width='30' height='15' alt='' /><br />");
				$option_tray->addElement($color_select);
				$option_tray->addElement($color_label);
				unset($color_select, $color_label);
				$i++;
	...
	case "save":
		...
		if($pollModuleHandler->getVar("version") < 140) {
			...
			$poll_obj->setVar("user_id", intval(@$_POST['user_id']));
	...
	case "update":
		...
		// new xoopspoll module
		if($pollModuleHandler->getVar("version") >= 140) {
		    $xpOptHandler =& xoops_getmodulehandler('option', $xoopsModuleConfig["poll_module"]);
			$xpLogHandler =& xoops_getmodulehandler('log', $xoopsModuleConfig["poll_module"]);
			$classRequest = ucfirst($xoopsModuleConfig["poll_module"]) . "Request";
			$classConstants = ucfirst($xoopsModuleConfig["poll_module"]) . "Constants";
			$notify = $classRequest::getInt('notify', $classConstants::NOTIFICATION_ENABLED, 'POST');
			$currentTimestamp = time();
			//$xuEndTimestamp   = method_exists('XoopsLocal', 'strtotime') ? XoopsLocal::strtotime($classRequest::getString('xu_end_time', null, 'POST'))
			//															 : strtotime($classRequest::getString('xu_end_time', null, 'POST'));
			$xuEndTimestamp   = strtotime($classRequest::getString('xu_end_time', null, 'POST'));
			$endTimestamp     = (empty($_POST['xu_end_time'])) ? ($currentTimestamp + $classConstants::DEFAULT_POLL_DURATION) : userTimeToServerTime($xuEndTimestamp);
			//$xuStartTimestamp = method_exists('XoopsLocal', 'strtotime') ? XoopsLocal::strtotime($classRequest::getString('xu_start_time', null, 'POST'))
			//															 : strtotime($classRequest::getString('xu_start_time', null, 'POST'));
			$xuStartTimestamp = strtotime($classRequest::getString('xu_start_time', null, 'POST'));
			$startTimestamp   = (empty($_POST['xu_start_time'])) ? ($endTimestamp - $classConstants::DEFAULT_POLL_DURATION) : userTimeToServerTime($xuStartTimestamp);
			...
			$poll_vars = array(
                 'user_id'     => $classRequest::getInt('user_id', $GLOBALS['xoopsUser']->uid(), 'POST'),
                 'question'    => $classRequest::getString('question', null, 'POST'),
                 'description' => $classRequest::getText('description', null, 'POST'),
                 'mail_status' => ($classConstants::NOTIFICATION_ENABLED == $notify) ? $classConstants::POLL_NOT_MAILED : $classConstants::POLL_MAILED,
                 'mail_voter'  => $classRequest::getInt('mail_voter', $classConstants::NOT_MAIL_POLL_TO_VOTER, 'POST'),
                 'start_time'  => $startTimestamp,
                 'end_time'    => $endTimestamp,
                 'display'     => $classRequest::getInt('display', $classConstants::DO_NOT_DISPLAY_POLL_IN_BLOCK, 'POST'),
                 'visibility'  => $classRequest::getInt('visibility', $classConstants::HIDE_NEVER, 'POST'),
                 'weight'      => $classRequest::getInt('weight', $classConstants::DEFAULT_WEIGHT, 'POST'),
                 'multiple'    => $classRequest::getInt('multiple', $classConstants::NOT_MULTIPLE_SELECT_POLL, 'POST'),
                 'multilimit'  => $classRequest::getInt('multilimit', $classConstants::MULTIPLE_SELECT_LIMITLESS, 'POST'),
                 'anonymous'   => $classRequest::getInt('anonymous', $classConstants::ANONYMOUS_VOTING_DISALLOWED, 'POST'),
			);
			...
			// now get the options
			$optionIdArray    = $classRequest::getArray('option_id', array(), 'POST');
			$optionIdArray    = array_map('intval', $optionIdArray);
			$optionTextArray  = $classRequest::getArray('option_text', array(), 'POST');
			$optionColorArray = $classRequest::getArray('option_color', array(), 'POST');
	...
	case "addmore":
		...
		// old xoopspoll or umfrage or any clone from them
			...
			for($i = 0; $i < 10; $i++) {
				...
				$color_select->addOptionArray($barcolor_array);
				$color_select->setExtra("onchange='showImgSelected(\"option_color_image[{$i}]\", \"option_color[{$i}]\", \"modules/{$xoopsModuleConfig['poll_module']}/images/colorbars\", \"\", \"" . XOOPS_URL . "\")'");
				$color_label = new XoopsFormLabel("", "<img src='" . $GLOBALS['xoops']->url("modules/{$xoopsModuleConfig['poll_module']}/images/colorbars/{$current_bar}") . "' name='option_color_image[{$i}]' id='option_color_image[{$i}]' class='alignbottom' width='30' height='15' alt='' /><br />");
				$option_tray->addElement($color_select);
				$option_tray->addElement($color_label);
	...
	case "restart_ok":
		...
		$end_time = (empty($_POST['end_time'])) ? 0 : ($_POST['end_time']);
		if (!empty($end_time)) {
			$timezone = (is_object($GLOBALS['xoopsUser'])) ? $GLOBALS['xoopsUser']->getVar("timezone") : null;
			$poll_obj->setVar("end_time", userTimeToServerTime(method_exists('XoopsLocal', 'strtotime') ? XoopsLocal::strtotime($end_time) : strtotime($end_time), $timezone));
		} else {
	...
	case "log":
		// new xoopspoll module
		if ($pollModuleHandler->getVar("version") >= 140) {
			redirect_header($GLOBALS['xoops']->url("modules/{$xoopsModuleConfig['poll_module']}/admin/main.php?op=log&amp;poll_id={$poll_id}"), 2, _MD_POLL_VIEWLOG);
		// old xoopspoll or umfrage or any clone from them
		} else {
			redirect_header($GLOBALS['xoops']->url("modules/{$xoopsModuleConfig['poll_module']}/admin/index.php?op=log&amp;poll_id={$poll_id}"), 2, _MD_POLL_VIEWLOG);
		}
		break;
} // switch
