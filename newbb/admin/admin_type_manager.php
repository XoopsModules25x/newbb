...
switch ($op) {
    case "save_type":
        $type_names = XoopsRequest::getString('type_name', '', 'POST');// $_POST['type_name'];
        $type_del   = array();
        foreach (array_keys($type_names) as $key) {
            ...
            if (XoopsRequest::getArraay("type_del[$key]", '', 'POST')) {
                $type_del[] = $key;
                continue;
            } else {
                foreach (array("type_name", "type_color", "type_description") as $var) {
                    $type_obj->setVar($var, XoopsRequest::getString($var[$key], '', 'POST'));
                }
