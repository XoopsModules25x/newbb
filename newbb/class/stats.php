    ...
    public function update($id, $type, $increment = 1)
    {
        ...
        if ($rows === 0) {
            $sql    = "    INSERT INTO {$this->table}" .
                      ...
                      "    VALUES " .
                      "        ('0', '{$increment}', '{$type}', '" . array_search("total", $this->param['period'], true) . "', NOW(), ''), " .
                      "        ('0', '{$increment}', '{$type}', '" . array_search("day", $this->param['period'], true) . "', NOW(), '%Y%j'), " .
                      "        ('0', '{$increment}', '{$type}', '" . array_search("week", $this->param['period'], true) . "', NOW(), '%Y%u'), " .
        ...
        if ($rows < 2 * count($this->param['period']) && !empty($id)) {
            $sql    = "    INSERT INTO {$this->table}" .
                      "        (`stats_id`, `stats_value`, `stats_type`, `stats_period`, `time_update`, `time_format`) " .
                      "    VALUES " .
                      "        ('{$id}', '{$increment}', '{$type}', '" . array_search("total", $this->param['period'], true) . "', NOW(), ''), " .
                      "        ('{$id}', '{$increment}', '{$type}', '" . array_search("day", $this->param['period'], true) . "', NOW(), '%Y%j'), " .
                      "        ('{$id}', '{$increment}', '{$type}', '" . array_search("week", $this->param['period'], true) . "', NOW(), '%Y%u'), " .
